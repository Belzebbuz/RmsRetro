...
public class ApiGrpcService(IClusterClient client) : ApiService.ApiServiceBase
{
	...
	public override Task<ActivateUserResponse> ActivateUser(Empty request, ServerCallContext context)
		=> client.GetGrain<IUserGrain>(Guid.NewGuid()).ActivateAsync();

	public override Task<GetUserStatusResponse> GetUserStatus(Empty request, ServerCallContext context)
		=> client.GetGrain<IUserGrain>(Guid.NewGuid()).GetStatusAsync();
}